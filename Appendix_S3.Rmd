...
<<processdata1>>
```
##STATISTICAL ANALYSES OF NODE AGES
...
##S3.1 Node age statstics based on the ancestral state estimation using Bayesian Binary Markov analysis in RASP.
```{r echo=FALSE}
...
<<BBMtreeplot>>
```
###Fig. S3.1.7 RASP BBM ancestral state reconstruction.
\newpage
From the previous plot we can group nodes as indicated in the legend.
```{r fig.width=7, fig.height=9, message=F, warning=F}
<<BBMstatsnodes>>
```
###Fig. S3.1.8 RASP BBM node groups.
\newpage
```{r echo=FALSE}
read_chunk('R_scripts/RASPBBM_node_age_stats.R')
```
...
<<processdata2>>
```
###S3.1.2 Comparing the average age of colonisation nodes on Réunion to those Mauritius
```{r}
<<BBMMauvsReu>>
<<BBMMauvsReuttest>>
```
...
<<processdata4>>
```
###S3.1.3 Comparing the age of colonisation events on Mauritius to *in situ* speciation events
...
<<processdata3>>
```
###S3.1.4 Comparing the age of *in situ* speciation events on Réunion to colonisation events
```{r}
<<BBMinsituReu>>
```
...
###S3.1.5 Comparing the age of flight loss nodes to nodes without flight loss
...
##S3.2 Node age statstics based on the ancestral state estimation using Maximum Likelihood analyses in R.
```{r echo=FALSE}
...
<<MLtreeplot>>
```
###Fig. S3.2.9 R Maximum Likelihood ancestral state reconstruction.
\newpage
From the previous plot we can group nodes as indicated in the legend.
```{r fig.width=7, fig.height=9, message=F, warning=F}
<<RMLstatsnodes>>
```
### Fig. S3.2.10 R Maximum Likelihood node groups.
\newpage
```{r echo=FALSE}
...
<<processdata6>>
```
###S3.2.2 Comparing the average age of colonisation nodes on Réunion to those Mauritius
```{r}
<<RMLMauvsReu>>
```
...
<<processdata8>>
```
###S3.2.3 Comparing the age of colonisation events on Mauritius to *in situ* speciation events
This is reversed relative to the test performed on Réunion nodes as there are many more *in situ* speciation nodes on Mauritius than colonisations.
```{r}
<<RMLinsituMau>>
```
...
<<processdata7>>
```
###S3.2.4 Comparing the age of *in situ* speciation events on Réunion to colonisation events
```{r}
<<RMLinsituReu>>
<<RMLinsituReuttest>>
```
...
###S3.2.5 Comparing the age of flight loss nodes to nodes without flight loss
```{r}
<<RMLflightloss>>
<<RMLflightlossttest>>
```
