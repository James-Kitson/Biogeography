...
beast.tree<-ggtree(beast, aes(color=location), ndigits = 2) +
  scale_fill_discrete() +
  geom_tiplab() +
  geom_text2(aes(label=posterior, subset=posterior>0.8), nudge_x = -0.3, nudge_y = 0.8) +
...
svg(file="BEAST/Tree_and_log_files/Relaxed_Biogeography/Beast_biogeography.svg", width=16, height=8)
beast.tree2 %>%
    scale_x_ggtree(breaks = seq(-12,0,1))
dev.off()

svg(file="BEAST/Tree_and_log_files/Relaxed_Biogeography/Beast_biogeography_pies.svg", width=16, height=8)
