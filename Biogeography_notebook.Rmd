...
beast.tree<-ggtree(beast, aes(color=location), ndigits = 2) +
  #scale_fill_discrete() +
  geom_tiplab() +
  geom_text2(aes(label=posterior, subset=posterior >= 0.80), nudge_x = -0.3, nudge_y = 0.8) +
  #geom_range(range = "height_0.95_HPD", color = "blue", size=2, alpha=0.3) +
  theme_tree2(legend.position = c(0.1,0.8), legend.text = element_text(size=rel(1.3)), legend.title = element_text(size=rel(1.5))) +
  xlim_tree(c(-12,4))
  
beast.tree2<-revts(beast.tree)

svg(file="BEAST/Tree_and_log_files/Relaxed_Biogeography/Beast_biogeography.svg", width=16, height=11)
beast.tree2 %>%
    scale_x_ggtree(breaks = seq(-12,0,1)) +
    scale_fill_discrete()
dev.off()

svg(file="BEAST/Tree_and_log_files/Relaxed_Biogeography/Beast_biogeography_pies.svg", width=16, height=11)
...
# We should also check that the BEAST and MrBayes Trees are compatible.

```{r}
library(phytools)

### read in the tree
MrBayes.trees<-read.nexus("MrBayes/Tree_files/Cratopus_Multilocus.nex.con.tre")
MrBayes.tree<-MrBayes.trees[[1]]
Beast.tree.ape <-read.nexus(file="BEAST/Tree_and_log_files/Relaxed_Biogeography/Biogeography_MCC.tree")
MrBayes.tree$tip.label<-names$Alt_label[match(MrBayes.tree$tip.label,names$Name)]
Beast.tree.ape$tip.label<-names$Alt_label[match(Beast.tree.ape$tip.label,names$Name)]

links<-as.matrix(cbind(Beast.tree.ape$tip.label,Beast.tree.ape$tip.label))

svg(file="BEAST/Tree_and_log_files/Relaxed_Biogeography/MrBayes_BEAST_cophylo.svg", width=15, height=8)
test<-cophylo(MrBayes.tree,Beast.tree.ape, rotate=TRUE)
plot(test)
#nodelabels.cophylo(which = 'left', text=MrBayes.tree$node.label)
#nodelabels.cophylo(which = 'right', text=Beast.tree.ape$node.label)
dev.off()
```
