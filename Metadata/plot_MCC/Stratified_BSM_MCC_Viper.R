...
### Clear the workspace
rm(list=ls())
setwd("~/Documents/Projects/Cratopus/Biogeography/Metadata/plot_MCC/")
...
### read in all the trees from MrBayes that have had the burnin removed and the branch lengths rescaled to mya.
#my.nexus.trees<-list.files("../Trees/", pattern = "*dated*")

### set how many trees per MrBayes replicated we want trees_per_file=10 = 10*10 trees to analyse
#trees_per_file<-10
...
#file.remove("BioGeoBEARs_save_files_final_model/Working_trees/My_newick.trees")
#file.remove("Output/colonisation_means.csv")

### for each MrBayes run pull out the number specified above random trees and write these to their own file
#for(i in my.nexus.trees){
#  my.tree.sample<-read.nexus(file=paste("../Trees/",i, sep=""))
#  for(j in sample(1:length(my.tree.sample), trees_per_file, replace = FALSE)){
#    sampled.tree<-my.tree.sample[j]
#  write.tree(sampled.tree, file="BioGeoBEARs_save_files_final_model/Working_trees/My_newick.trees", append = TRUE)
#  }
#}
...
#my.newick.trees<-read.tree(file="BioGeoBEARs_save_files_final_model/Working_trees/My_newick.trees")

#####provide a path to the geography file
geogfn <- "../BioGeoBEARS_islands_old.txt"
###Look at your geographic range data:
tipranges = getranges_from_LagrangePHYLIP(lgdata_fn=geogfn)
### provide a path to the distance matrix
dist.mat<-"../BioGeoBEARS_dist_scale_strat.txt"
### provide a path to the time periods file
eras<-"../timeperiods.txt"
...
trfn<-"BioGeoBEARs_save_files_final_model/Working_trees/All_dating_combined_MCC.newick"
...
### for each tree in my.newick.trees, perform the DEC+J+X analysis
    my.tree<-read.tree(trfn)
    ...
    analysis_titletxt ="BioGeoBEARS DEC+J+X on Cratopus MrBayes tree"
  ...
  for(z in 1:length(clado_events_tables)){
    ### Pull ages of colonisation nodes from each BSM
    BSM_nodes<-as.data.frame(cbind(clado_events_tables[[z]]$node,
                                   clado_events_tables[[z]]$clado_event_txt,
                                   rep(NA, nrow(clado_events_tables[[z]])),
                                   clado_events_tables[[z]]$time_bp,
                                   clado_events_tables[[z]]$node.type))
    ...
    BC<-mean(as.numeric(processed_nodes$V4[which(processed_nodes$V2=="B" & grepl("C",processed_nodes$V3))]))
    CB<-mean(as.numeric(processed_nodes$V4[which(processed_nodes$V2=="C" & grepl("B",processed_nodes$V3))]))
    
    next_line<-cbind(k,z,BC,CB)
    
    colonisation_means<-rbind(colonisation_means, next_line)
    
    colonisation_means<-as.data.frame(colonisation_means)
    
    colonisation_means_subs<-subset(colonisation_means, BC >0 & CB >0, select=c(2,3))
    
    write.csv(colonisation_means_subs, file="Output/colonisation_means.csv")
    
    ### THIS ENDS THE PROCESS FOR TREE K, START AGAIN
    }
### THIS ENDS THE COMPLETE ANALYSIS
}

t.test(colonisation_means_subs$BC,colonisation_means_subs$CB, paired=TRUE)
